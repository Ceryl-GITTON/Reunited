...
void main() async {
  ...
  runApp(const ReunitedCountdownWebApp());
}

class ReunitedCountdownWebApp extends StatefulWidget {
  const ReunitedCountdownWebApp({super.key});

  @override
  State<ReunitedCountdownWebApp> createState() => _ReunitedCountdownWebAppState();
}

class _ReunitedCountdownWebAppState extends State<ReunitedCountdownWebApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: CountdownWebScreen(onLanguageChange: _changeLanguage),
    );
  }
}

class CountdownWebScreen extends StatefulWidget {
  final Function(Locale) onLanguageChange;

  const CountdownWebScreen({super.key, required this.onLanguageChange});

  @override
  State<CountdownWebScreen> createState() => _CountdownWebScreenState();
}

class _CountdownWebScreenState extends State<CountdownWebScreen>
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 10),
                    // Lien vers APK mobile
                    TextButton.icon(
                      onPressed: () {
                        // Ouvrir le lien vers les releases GitHub
                        // Pour le web, on peut utiliser dart:html mais pour la simplicité, on affiche juste un message
                        showDialog(
                          context: context,
                          builder: (context) => AlertDialog(
                            ...
                            content: const Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Text('Pour utiliser les widgets sur votre téléphone Android, téléchargez l\'APK depuis :'),
                                SizedBox(height: 10),
                                SelectableText(
